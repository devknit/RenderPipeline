Shader "Hidden/Dof/DepthOfFieldHdr"
{
	...
	void vertFlip( appdata_img v, out v2f o)
	{
		o.pos = UnityObjectToClipPos( v.vertex);
		o.uv1.xy = v.texcoord.xy;
		o.uv.xy = v.texcoord.xy;
	...
	#if UNITY_UV_STARTS_AT_TOP
		o.uv.y = (_MainTex_TexelSize.y < 0)? 1.0 - o.uv.y : o.uv.y;
		o.uv1.y = (_MainTex_TexelSize.y < 0)? 1.0 - o.uv1.y : o.uv1.y;
	#endif			
	} 
	void vertBlurPlusMinus( appdata_img v, out v2fBlur o)
	{
		o.pos = UnityObjectToClipPos(v.vertex);
	...
	float4 fragBlurAlphaWeighted( v2fBlur i) : SV_Target 
	{
		...
		sum.a = sampleA.a;
		sum.rgb = (sampleA.a < 1e-2f)? sampleA.rgb : sum.rgb;
	...
	fixed4 fragCaptureCoc( v2f i) : SV_Target 
	{	
	//	fixed4 color = float4( 0, 0, 0, 0); //tex2D( _MainTex, i.uv1.xy);
